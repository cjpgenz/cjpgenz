...
export default function Header({ simple = false }: HeaderProps) {
  return (
    <header className="bg-[rgba(244,235,215,0.94)] backdrop-blur-sm border-b-2 border-ink sticky top-0 z-100">
      <div className={`max-w-[1280px] mx-auto px-5 sm:px-8 lg:px-[56px] py-4 grid ${simple ? 'grid-cols-[1fr_auto]' : 'grid-cols-[1fr_auto] lg:grid-cols-[auto_1fr_auto]'} items-center gap-4 lg:gap-10`}>
        ...
        <Link
          href={simple ? "/sack" : "/#join"}
          target={simple ? "_blank" : undefined}
          rel={simple ? "noopener noreferrer" : undefined}
        ...
        >
          {simple ? "Sign Petition →" : "Join the Party"}
        </Link>
      </div>
    </header>
  );
